...
Double_t BruteForce( Double_t spacing, Double_t &eta_0, Double_t &eta_1, Double_t chi2, TMatrixD *matrix_array ){
	...
	Double_t diff = 0.5*(UB_0 - LB_0)*(1/STEP_SIZE_I)*spacing;		// Defines the range over which you search (+/- diff from estimates)
	Double_t lb_0 = 0.0, ub_0 = 0.0, lb_1 = 0.0, ub_1 = 0.0;

	// Define the upper and lower bounds for each eta
	if ( diff < eta_0 ){ lb_0 = ( eta_0 - diff >= LB_0 ? eta_0 - diff : LB_0 ); }
	ub_0 = ( eta_0 + diff <= UB_0 ? eta_0 + diff : UB_0 );
	if ( diff < eta_1 ){ lb_1 = ( eta_1 - diff >= LB_1 ? eta_1 - diff : LB_1 ); }
	ub_1 = ( eta_1 + diff <= UB_1 ? eta_1 + diff : UB_1 );
