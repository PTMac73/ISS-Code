...
void DrawEVZ( TTree *t, plotterOptions &opt_s, Int_t pos_number ){
	...
	if ( DRAW_THEORETICAL_LINES_NR == 1 ){
		...
		Double_t rho_0 = ISSArrayRadius( -6.0, 6.0, 14.0 );							// Array radius in mm - this is estimated using width of Si strips
	...
	for ( Int_t i = 0; i < opt_s.numIter; i++ ){
		...
		if ( SWITCH_PRINT_CANVAS == 1 ){
			c_evz[i]->Print( Form( "%s/EVZ/EVZ_%i%s", opt_s.printDir.Data(), i, PRINT_FORMAT.Data() ) );
