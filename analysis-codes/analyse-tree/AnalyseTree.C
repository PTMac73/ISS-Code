...
*/
		...
		if ( is_in_used_det && is_in_rdt_and_td_total && is_in_theta_custom && is_in_xcal ){
			if ( SW_EX[0] == 1 ){
				...
				if ( DET_BY_DET == 1 ){ h_ex_dbd[i]->Fill( Ex[i]*gain_match_pars[ARR_POSITION-1][0] + gain_match_pars[ARR_POSITION-1][1] ); }
			}
