...
// 2 = d(82Se,p)83Se
Int_t reaction = 2;
const Int_t NUM_REACTIONS = 3;

TString reaction_string_arr[3] = {
	"d(28Mg,p)29Mg",
	"d(28Si,p)29Si",
	"d(82Se,p)83Se"
};
...
void Set82SeDP( ReactionParameters &rp ){
	rp.q = 1;												// Charge of outgoing proton in units of e
	rp.B = 2.85*c*(1e-8);									// Convert B-field to MeV / (cm*c*e)
	rp.m1 = MassExcessToNuclearMass( -77.5938, 82, 22 ); 	// m1 = 82Se in MeV / c^2		(beam)
	rp.m2 = MassExcessToNuclearMass(  13.1357,  2,  1 );	// m2 = d in MeV / c^2			(target)
	rp.m3 = MassExcessToNuclearMass(  7.28890,  1,  1 );	// m3 = p in MeV / c^2			(ejectile)
	rp.m4 = MassExcessToNuclearMass( -75.3405, 83, 22 );	// m4 = 83Se in MeV / c^2		(recoil)
	rp.bepn = 10.012;										// bepn = beam energy per nucleon in MeV/u
	return;
}
...
void FillRP( ReactionParameters &rp ){
	switch(reaction){
		...
		case 2 :
			...
			break;
		default:
			std::cout << "Reaction #" << reaction << "is invalid.\n";
			std::exit(1);
	}
	return;
}
...
void PrintRP( ReactionParameters &rp ){
	Int_t w = 12;
	std::cout << 
		std::left << std::setw(w) << "q" << rp.q << "\n" <<
		std::left << std::setw(w) << "B" << rp.B/(c*(1e-8)) << " T\n" << 
		std::left << std::setw(w) << "m1" << rp.m1 << " = " << rp.m1/u << " u\n" << 
		std::left << std::setw(w) << "m2" << rp.m2 << " = " << rp.m2/u << " u\n" <<  
		std::left << std::setw(w) << "m3" << rp.m3 << " = " << rp.m3/u << " u\n" <<  
		std::left << std::setw(w) << "m4" << rp.m4 << " = " << rp.m4/u << " u\n" << 
		std::left << std::setw(w) << "bepn" << rp.bepn << " MeV/u\n";
	return;
}
