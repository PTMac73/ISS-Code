...
FitResult_t fit1_full[NUM_POSI];	// For calibrating positions to energy
...
void EnergyCalibration(){
	...
	TGraphErrors* g_ecal_full[NUM_POSI];
	TCanvas* c_ecal_full[NUM_POSI];
	...
	for ( Int_t i = 0; i < NUM_POSI; i++ ){
		...
		// CALCULATE CALIBRATION FOR THE FULL SPECTRUM
		// Define arrays to hold the variables from the data table
		...
		for ( Int_t k = 0; k < NUM_STAT; k++ ){
			x[k]     = data_table_pos1_full[i][k][0];
			x_err[k] = data_table_pos1_full[i][k][1];
			y[k]     = data_table_pos1_full[i][k][2];
			y_err[k] = data_table_pos1_full[i][k][3];
		}
		...
		// Create TGraphs and TCanvases and format them
		g_ecal_full[i] = new TGraphErrors( NUM_STAT, x, y, x_err, y_err );
		FormatGraph( g_ecal_full[i] );
		g_ecal_full[i]->SetTitle( Form( "Position %i | FULL", i + 1 ) );
		
		c_ecal_full[i] = new TCanvas( Form( "c_ecal_full_%i", i + 1 ), "Calibrate position to energy fits", C_WIDTH, C_HEIGHT );
		c_ecal_full[i]->SetCanvasSize(C_WIDTH,C_HEIGHT);
		GlobSetCanvasMargins( c_ecal_full[i], 0.1, 0.02, 0.1, 0.1 );
		g_ecal_full[i]->Draw("AP");
		
		
		// Get straight line parameters
		TFitResultPtr s = g_ecal_full[i]->Fit(sline, "SQ");
		if ( swVERBOSE ){ std::cout << "\u03c7^2 = " << s->Chi2() << "\n"; }
		TMatrixDSym* covmc = (TMatrixDSym*)s->GetCovarianceMatrix().Clone();
		SetFitParameters( sline->GetParameter(1), sline->GetParameter(0), covmc, fit1_full[i] );
		if ( swVERBOSE ){ PrintFitParameters( fit1_full[i] ); }
	...
	std::cout << "\nFull fit parameters:\n";
	std::cout
		<< std::setw(w) << fit1_full[0].m << sp 
		<< std::setw(w) << fit1_full[0].c << sp
		<< std::setw(w) << fit1_full[1].m << sp 
		<< std::setw(w) << fit1_full[1].c << "\n";
	...
	std::cout << "\nFull fit parameters:\n";
	std::cout 
		<< std::setw(w) << TMath::Sqrt( (*fit1_full[0].covmc)(1,1) ) << sp
		<< std::setw(w) << TMath::Sqrt( (*fit1_full[0].covmc)(0,0) ) << sp
		<< std::setw(w) << (*fit1_full[0].covmc)(1,0) << sp
		<< std::setw(w) << TMath::Sqrt( (*fit1_full[1].covmc)(1,1) ) << sp
		<< std::setw(w) << TMath::Sqrt( (*fit1_full[1].covmc)(0,0) ) << sp
		<< std::setw(w) << (*fit1_full[1].covmc)(1,0) << sp << "\n\n\n";


	// Print the canvas
	c_ecal->Print("energy_cal_fits.pdf");
	c_ecal_full[0]->Print("energy_cal_full_pos1.pdf");
	c_ecal_full[1]->Print("energy_cal_full_pos2.pdf");
