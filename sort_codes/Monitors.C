...
TCanvas *cCanvas;
TCanvas *cEX;
TCanvas *sharpCanvas, *sharpCanvas2;
...
TH2F* hecalVxcal[24];
TH2F* hecalVz;
TH2F* EVZ; 			// E v.s. z plot for Sharpy
TH2F* hecalVzR;
TH2F* hrdt[4];
TH2F* hrdtg[4];
TH2F* helum[2];
...
TH1F* h0e;
TH1F* h0tac;
TH1F* EXE;			// Excitation energy graph for Sharpy
  ...
  if (ProcessedEntries<NUMSORT) {
    ...
    for (Int_t i = 0; i < 24; i++) {
      ...
      if (x[i]>-1.1&&x[i]<1.1&&e[i]>100&&(xn[i]>0||xf[i]>0)) {
        ...
        //EVZ->Fill(z[i],ecrr[i]);										// SHARPY GRAPH
...
{
	// Now plot E v.s. z
	sharpCanvas = new TCanvas("sharpCanvas","E v.s. z", 1080, 810);
	EVZ->Draw("scat");
	EVZ->Draw("colz same");
	sharpCanvas->Print("EVZ.png");
	
	// Plot excitation energy
	sharpCanvas2 = new TCanvas( "sharpCanvas2","Excitation energy spectrum", 1080, 810 );
	EXE->Draw();
	sharpCanvas2->Print("EXE.png");
	
	
	/*
	TFile *_file0 = new TFile("../root_data/hist1.root","NEW");
	EVZ->Write();
	_file0->Close();
	*/
